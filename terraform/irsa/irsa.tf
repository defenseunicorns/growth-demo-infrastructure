...
resource "aws_secretsmanager_secret" "public_key" {
  name                    = "${var.environment}-oidc-public-key"
...
resource "aws_secretsmanager_secret_version" "public_key" {
  ...
  secret_id     = "${var.environment}-oidc-public-key"
  secret_string = tls_private_key.keypair.public_key_pem
}

resource "aws_secretsmanager_secret" "private_key" {
  name                    = "${var.environment}-oidc-private-key"
...
resource "aws_secretsmanager_secret_version" "private_key" {
  ...
  secret_id     = "${var.environment}-oidc-private-key"
...
data "external" "key_id" {
  program = ["bash", "${path.module}/key_id.sh", tls_private_key.keypair.public_key_pem]
}

data "external" "key_modulus" {
  program = ["bash", "${path.module}/key_modulus.sh", tls_private_key.keypair.public_key_pem]
}
